...
import { FaSave, FaUserEdit, FaTrash } from 'react-icons/fa'
import { InstructorAPI } from '../../api' // ✅ ใช้ API รวม
...
export default function InstructorProfile() {
  ...
  const loadProfile = async () => {
    try {
      const res = await InstructorAPI.getProfile(token)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await InstructorAPI.updateProfile(payload, token)
  ...
  const handleDeleteAccount = async () => {
    try {
      await InstructorAPI.deleteProfile(token)
  ...
  return (
    <div className="container py-5">
      <h2 className="fw-bold mb-4 text-primary">
        <FaUserEdit className="me-2" /> Instructor Profile
      </h2>
